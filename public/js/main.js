 ...
 *
 *  SEÇÕES:
 *  1. AUTH HELPERS   — Retorna os headers com o token JWT do usuário logado.
 *  2. STATE          — Variáveis de estado global (usuário, fases, tasks, etc.).
 *  3. UTILS          — Funções utilitárias (formatação de datas, cores de fases, escape HTML).
 *  4. TOAST          — Notificações visuais (popup no canto inferior direito).
 *  5. AUTH           — Login, logout, validação de sessão e inicialização do dashboard.
 *  6. SSE            — Server-Sent Events: recebe atualizações em tempo real do servidor.
 *  7. DATA FETCH     — Busca dados de fases, membros e etiquetas da API.
 *  8. MEMBERS DRAWER — Painel lateral de membros da equipe.
 *  9. BOARD RENDER   — Renderiza as colunas e cards do Kanban.
 * 10. DRAG & DROP    — Lógica de arrastar cards entre colunas.
 * 11. CREATE MODAL   — Modal para criação de novas tasks.
 * 12. DETAILS MODAL  — Modal de detalhes/edição de tasks existentes.
 * 13. LABELS         — Sistema de etiquetas (criação, aplicação, remoção).
 * 14. CHECKLISTS     — Etapas de execução das tasks.
 * 15. COLUMNS        — Adicionar/renomear/excluir fases do Kanban.
 * 16. FILTERS        — Filtros por usuário, etiqueta e texto.
 * 17. PHASE LOGS     — Histórico de movimentações por fase (accordion).
 * 18. COMMENTS       — Envio e renderização de comentários da timeline.
 * 19. MODAL HELPERS  — Abrir/fechar modais genéricos.
 * 20. INIT           — Inicialização: checa autenticação ao carregar a página.
 */

  /** @returns {Object} Headers com Authorization Bearer e Content-Type JSON. */
      ...
      filteredUsers.forEach(user => {
        ...
        user.actions.forEach(act => {
          ...
          if (t === 'created')             text = 'Criou a task';
            ...
            else if (ca === 'status_changed')  text = 'Status de "' + title + '" â†’ <strong>' + esc(det.to_value || '') + '</strong>';
          ...
          actEl.innerHTML = '<span class="action-dot">â€¢</span>' + text + '<span class="action-entry-time">' + timeFmt + '</span>';
  ...
  // ============================================================
  // ADVANCED CHECKLISTS (ETAPAS DE EXECUÃ‡ÃƒO)
  // ============================================================
    ...
    currentTaskChecklists.forEach(chk => {
      ...
      if (chk.activity && chk.activity.length > 0) {
        activityHtml = chk.activity.map(a => {
          ...
          else if (a.action === 'status_changed')  at = 'Status â†’ <strong>' + esc(a.to_value) + '</strong>';
          ...
          return '<div style="padding:2px 0;border-bottom:1px dashed rgba(255,255,255,0.03);font-size:10.5px;color:var(--text-muted);">â€¢ ' + at + (t2 ? ' <span style="font-size:8px;color:var(--text-dim);">' + t2 + '</span>' : '') + '</div>';
        }).join('');
      }
...
// ==========================================
// 21. DASHBOARD — TABS, METRICS & CHARTS
// ==========================================

/** Instâncias de Chart.js para destruir ao recriar */
let pieChartInstance = null;
let barChartInstance = null;

/**
 * Alterna entre as abas do painel (Kanban, Métricas, Gamificação).
 * Controla visibilidade das seções e carrega dados sob demanda.
 */
function switchTab(tab) {
  ...
  if (filterBar) filterBar.style.display = tab === 'kanban' ? 'flex' : 'none';

  // New Task button only in Kanban
  const newTaskBtn = document.getElementById('new-task-btn');
  if (newTaskBtn) newTaskBtn.style.display = tab === 'kanban' ? 'inline-flex' : 'none';

  // Members toggle only in Kanban
  const membersBtn = document.getElementById('members-toggle-btn');
  if (membersBtn) membersBtn.style.display = tab === 'kanban' ? 'inline-flex' : 'none';

  // Load data on demand
  if (tab === 'metrics') loadMetricsDashboard();
  if (tab === 'gamification') loadGamificationDashboard();
}

/**
 * Carrega os dados de métricas da API e renderiza os cards e gráficos.
 */
async function loadMetricsDashboard() {
  try {
    // Overview
    const overviewRes = await fetch('/api/stats/overview');
    if (!overviewRes.ok) throw new Error('Erro ao carregar resumo de metricas.');
    const overviewData = await overviewRes.json();
    const ov = overviewData.overview || {};

    document.getElementById('metric-total-hours').textContent = `${ov.totalHours || 0}h`;
    document.getElementById('metric-week-hours').textContent = `${ov.hoursLast7Days || 0}h`;
    document.getElementById('metric-tasks-delivered').textContent = ov.tasksDelivered || 0;
    document.getElementById('metric-tasks-blocked').textContent = ov.tasksBlocked || 0;
    document.getElementById('metric-bottlenecks').textContent = ov.bottlenecks || 0;
    document.getElementById('metric-active-members').textContent = ov.activeMembers || 0;

    if (typeof Chart === 'undefined') {
      throw new Error('Chart.js nao foi carregado.');
    }

    // Phase Names for chart labels
    const phaseNameMap = {
      backlog: 'Backlog', todo: 'TO-DO', andamento: 'Em Andamento',
      revisao: 'Em Revisão', concluido: 'Concluído', bloqueado: 'Bloqueado'
    };
    const phaseColorMap = {
      backlog: '#64748b', todo: '#3b82f6', andamento: '#eab308',
      revisao: '#a855f7', concluido: '#10b981', bloqueado: '#ef4444'
    };

    // Pie Chart (Time by Phase)
    const pieData = (overviewData.charts?.timeByPhase || []);
    if (pieChartInstance) pieChartInstance.destroy();
    const pieCtx = document.getElementById('chart-pie-phases')?.getContext('2d');
    if (pieCtx) {
      pieChartInstance = new Chart(pieCtx, {
        type: 'doughnut',
        data: {
          labels: pieData.map(p => phaseNameMap[p.phase] || p.phase),
          datasets: [{
            data: pieData.map(p => parseFloat(p.hours)),
            backgroundColor: pieData.map(p => phaseColorMap[p.phase] || '#6366f1'),
            borderWidth: 0,
            hoverOffset: 8
          }]
        },
    ...
    // Bar Chart (7-day history)
    const barData = (overviewData.charts?.history7Days || []);
    if (barChartInstance) barChartInstance.destroy();
    const barCtx = document.getElementById('chart-bar-week')?.getContext('2d');
    if (barCtx) {
      ...
      barChartInstance = new Chart(barCtx, {
        ...
        data: {
          labels: barData.map(d => {
            ...
            return weekdayNames[dt.getDay()] + ' ' + dt.getDate();
          }),
          datasets: [{
            label: 'Horas',
            data: barData.map(d => parseFloat(d.hours)),
        ...
        options: {
          ...
          maintainAspectRatio: true,
          scales: {
            y: { beginAtZero: true, ticks: { color: '#64748b', font: { family: 'Inter', size: 10 } }, grid: { color: 'rgba(255,255,255,0.04)' } },
    ...
    // Team Table
    const teamRes = await fetch('/api/stats/team');
    if (!teamRes.ok) throw new Error('Erro ao carregar ranking da equipe.');
    const teamData = await teamRes.json();
    const tbody = document.getElementById('team-stats-tbody');
    if (tbody) {
      if (!teamData || teamData.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-dim);">Nenhum dado de tempo registrado ainda.</td></tr>';
      } else {
        tbody.innerHTML = teamData.map(member => `
          <tr>
            <td>${esc(member.name || 'Desconhecido')}</td>
            <td>${member.totalHours}h</td>
            <td>${member.tasksCompleted || 0}</td>
            <td>${member.tasksTouched || 0}</td>
            <td>
              <strong>${esc(member.currentStatus || 'Livre')}</strong>
              ${member.currentTaskTitle ? `<div style="color:var(--text-dim);font-size:11px;margin-top:2px;">#${esc(member.currentTaskId)} ${esc(member.currentTaskTitle)}</div>` : ''}
            </td>
          </tr>
        `).join('');
      }
    ...
    const impedimentsRes = await fetch('/api/stats/impediments');
    if (!impedimentsRes.ok) throw new Error('Erro ao carregar impedimentos.');
    const impediments = await impedimentsRes.json();
    const impedimentsTbody = document.getElementById('impediments-tbody');
    if (impedimentsTbody) {
      if (!impediments || impediments.length === 0) {
        impedimentsTbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-dim);">Nenhum impedimento relevante no momento.</td></tr>';
      } else {
        impedimentsTbody.innerHTML = impediments.map(task => `
          <tr>
            <td><strong>#${esc(task.id)}</strong> ${esc(task.title || 'Task')}</td>
            <td>${esc(task.phaseName || task.phase || '-')}</td>
            <td>${esc(task.assigneeName || 'Sem responsavel')}</td>
            <td>${task.totalHours || 0}h</td>
            <td>${fmtRelativeTime(task.updatedAt || task.createdAt)}</td>
          </tr>
        `).join('');
      }
    }
  } catch (error) {
    console.error('Erro ao carregar dashboard de métricas:', error);
    toast(error.message || 'Erro ao carregar metricas', 'error');
  }
}

/**
 * Carrega o leaderboard de gamificação (XP/Níveis) direto do Bot do Discord.
 */
async function loadGamificationDashboard() {
  const BOT_URL = window.CHDEAL_BOT_URL || `${window.location.protocol}//${window.location.hostname}:3005`;
  const tbody = document.getElementById('gamification-tbody');
  if (!tbody) return;

  try {
    const res = await fetch(`${BOT_URL}/gamification/leaderboard`);
    if (!res.ok) throw new Error('Bot offline');
    const json = await res.json();
    const data = json.data || [];

    if (data.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;color:var(--text-dim);">Nenhum dado de gamificação disponível.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = data.map((user, i) => `
      <tr>
        <td>${medals[i] || (i + 1)}</td>
        <td>
          <strong>${esc(user.name || user.discordId || 'Jogador')}</strong>
          <div class="xp-bar-container">
            <div class="xp-bar-fill" style="width: ${Math.min(100, (user.xp / (user.nextLevelXp || 1000)) * 100)}%"></div>
          </div>
        </td>
        <td>${esc(user.levelName || 'Nivel ' + (user.level || 1))}</td>
        <td><strong>${user.xp || 0}</strong> XP</td>
      </tr>
    `).join('');
  } catch (error) {
    console.warn('Gamification endpoint não disponível:', error.message);
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--text-dim);">
      <i class="fa-solid fa-wifi" style="margin-right:6px;"></i>
      O Bot do Discord precisa estar rodando para exibir os dados de gamificação.
    </td></tr>`;
  }
}
